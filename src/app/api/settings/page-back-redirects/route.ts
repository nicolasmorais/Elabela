...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const result = await client.query('SELECT value FROM settings WHERE key = $1', ['pageBackRedirects']);
...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    await client.query(
      ...
      ['pageBackRedirects', JSON.stringify(redirects)]
    );
    
    return NextResponse.json({ message: 'Configurações de redirecionamento salvas!' });
  } catch (error) {
